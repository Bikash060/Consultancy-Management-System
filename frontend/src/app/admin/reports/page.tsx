...
import { Card, StatCard, ChartCard, DonutChart, BarChart, AreaChart, MetricCard } from '@/components/ui';
...
export default function ReportsPage() {
    ...
    const loadData = async () => {
        try {
            const [counselorRes, countryRes] = await Promise.all([
                adminApi.getReports(),
    ...
    const avgSuccessRate = counselorReports.length > 0
        ? counselorReports.reduce((sum, r) => sum + r.success_rate, 0) / counselorReports.length
        : 0;

    // Prepare chart data
    const countryChartData = countryStats.slice(0, 8).map(cs => ({
        name: cs.country,
        applications: cs.count,
    }));
    ...
    const monthlyTrend = [
        { name: 'Jan', applications: 15, approved: 10 },
        { name: 'Feb', applications: 22, approved: 16 },
        { name: 'Mar', applications: 18, approved: 14 },
        { name: 'Apr', applications: 28, approved: 22 },
        { name: 'May', applications: 32, approved: 25 },
        { name: 'Jun', applications: 35, approved: 28 },
    ];
        ...
        <div className="space-y-6">
            ...
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-emerald-600 via-teal-600 to-cyan-600 p-6 md:p-8 text-white shadow-xl">
                <div className="relative z-10">
                    ...
                    <p className="text-white/80 max-w-lg">
                        Comprehensive performance insights, trends, and statistics for your consultancy.
                    </p>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <MetricCard
                    ...
                    color="blue"
                    icon={<FolderIcon />}
                    sparklineData={[8, 12, 18, 22, 28, totalApplications]}
                />
                <MetricCard
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <ChartCard
                    ...
                    subtitle="Geographic distribution of visa applications"
                >
                    {countryChartData.length > 0 ? (
                        ...
                        <EmptyState message="No country data available" icon="🌍" />
                    )}
                </ChartCard>

                {/* Performance Distribution */}
                <ChartCard
                    title="Counselor Performance Distribution"
                    subtitle="Success rate categories across team"
                >
                    {performanceDistribution.length > 0 ? (
                        <DonutChart data={performanceDistribution} height={300} innerRadius={70} />
                    ) : (
                        <EmptyState message="No performance data" icon="📊" />
                    )}
                </ChartCard>
            </div>
            ...
            <ChartCard
                ...
                subtitle="Applications submitted and approved over the last 6 months"
            >
                <AreaChart
                    data={monthlyTrend}
                    dataKeys={[
            ...
            {counselorChartData.length > 0 && (
                <ChartCard
                    title="Counselor Comparison"
                    subtitle="Applications handled vs approved by counselor"
                >
                    <BarChart
            ...
            {/* Detailed Counselor Performance */}
            <Card>
                ...
                {counselorReports.length > 0 ? (
                                ...
                                {counselorReports.map((report, idx) => (
                                    <tr key={report.counselor.id} className="border-b border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="font-medium text-slate-900 dark:text-white">
                                                        {report.counselor.profile?.first_name} {report.counselor.profile?.last_name}
                                                    </p>
                                        ...
                                        <td className="py-4 px-4 text-center">
                                            <span className="text-slate-900 dark:text-white font-medium">{report.total_applications}</span>
                                        </td>
                                        <td className="py-4 px-4 text-center">
                                            <span className="text-emerald-600 dark:text-emerald-400 font-medium">{report.visa_approved}</span>
                                        </td>
                                        <td className="py-4 px-4 text-center">
                                            <span className={`px-2.5 py-1 rounded-full text-sm font-medium ${report.success_rate >= 70
                                        ...
                                        <td className="py-4 px-4 w-48">
                                            <div className="flex items-center gap-2">
                                                <div className="flex-1 h-2.5 bg-slate-200 dark:bg-slate-700 rounded-full overflow-hidden">
                                                    <div
                                                        className={`h-full rounded-full transition-all duration-500 ${report.success_rate >= 70 ? 'bg-gradient-to-r from-emerald-500 to-teal-500'
                                                                : report.success_rate >= 50 ? 'bg-gradient-to-r from-amber-500 to-orange-500'
                    ...
                    <EmptyState message="No counselor data available" icon="👥" />
                )}
            </Card>
        </div>
    );
...
function FolderIcon() {
    return (
        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" className="w-5 h-5 text-blue-600 dark:text-blue-400">
...
function CheckIcon() {
    return (
        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" className="w-5 h-5 text-emerald-600 dark:text-emerald-400">
...
function TrendIcon() {
    return (
        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" className="w-5 h-5 text-purple-600 dark:text-purple-400">
